...
# picasa
BASE_DIR = '/scratch/csaee/paper/picasa/'
...
class Basic(object):
    ...
    def unfold(self,P):
        ...
        for i in range(len(P)-1):
            fil1=P_unfold[i]
            fil2=P_unfold[i+1]
            P_unfold[i+1]=np.subtract(fil2, fil1-self.lbox/2)%self.lbox-self.lbox/2+fil1
        return P_unfold
...
class ComputeCurvature(Basic):
    ...
    def kappa(self, P):
        ...
        mu_f=np.zeros(len(P)-1)
        mu_b=mu_f.copy()

        dP=P[1:]-P[:-1]
        l0=np.linalg.norm(dP, axis=1)
        for i in range(len(mu_b)):
            if (i<len(mu_b)-1):
                mu_f[i]=np.dot(dP[i], dP[i+1])/(l0[i]*l0[i+1])
            if (i>0):
                mu_b[i]=np.dot(dP[i], dP[i-1])/(l0[i]*l0[i-1])
        mu_f[mu_f>1]=1
        mu_f[mu_f<-1]=-1 # removing numerical errors

        mu_b[mu_b>1]=1
        mu_b[mu_b<-1]=-1

        kappa_f=np.arccos(mu_f)/l0
        kappa_b=np.arccos(mu_b)/l0
...
class ExtractProfiles(Basic):
    ...
    def straighten(self,Pos, P, rbins, Vel=None,sort=None, Num_cum=None, Num_W=None ):
        ...
        for i in range(len(filx)-1):
            ...
            if(self.pbc is True):
                fil2=np.subtract(fil2, fil1-self.lbox/2)%self.lbox-self.lbox/2+fil1 #accounting for PBC, unfolding
            ...
            if(self.pbc is True):
                Pos2=(np.subtract(np.subtract(Pos2, (fil1-self.lbox/2))%self.lbox, self.lbox/2))
                fil2=(np.subtract(np.subtract(fil2, (fil1-self.lbox/2))%self.lbox, self.lbox/2))
                fil1=(np.subtract(np.subtract(fil1, (fil1-self.lbox/2))%self.lbox, self.lbox/2))
            else:
    ...
    def estimate_profile_curve(self,Pos, P, rmin, rmax, dlogr=None, dr=None,
        ...
        return OUT
    def caustic(self,Pos_straight, rmin, rmax, lfil, Vel_straight, dlogr=None, dr=None, N_r=50,N_v=40, N_phi=20,log=True, vmax=2000):
        """ gives the 3D histogram in vr, r, phi of the filament. This can be used to identify caustics.
        inputs:
        Pos_straight: positions of particles in the box, aligned such that the spine of the filament is along the z-axis shape is [Np, 3]
        rmin, rmax: bounds on radial bins
        lfil: length of the filament
        Vel_straight: velocities of the straightened filament
        dlogr (dr): radial spacing for log (linear) radial bins
        N_r: number of radial bins (default is 50)
        N_v: number of vr bins (default is 40)
        N_phi: number of azimuthat bins (default is 20)
        log: True for logarithmic bins in r, False for linear. Default is True.
        vmax: vr bins are from -vmax to vmax.
        output:
        Dictionary with the following keys:
        Hist3D: of shape [N_r, N_v, N_phi]
        rbins, vbins, phibins"""

        RB=self.create_rbins(rmin, rmax, dlogr, dr, N_r, log)
        rbins=RB["rbins"]

        phibins=np.linspace(0,np.pi, N_phi+1)
        vbins=np.linspace(-vmax, vmax, N_v+1)

        r_perp=np.sqrt(Pos_straight[:,0]**2+Pos_straight[:,1]**2)
        phi=np.arctan2(Pos_straight[:,1], Pos_straight[:,0])
        Velr=(Vel_straight[:,0]*Pos_straight[:,0]+Vel_straight[:,1]*Pos_straight[:,1])/r_perp #radial velocity
        ip=np.array([r_perp, Velr, phi]).T
        O=np.histogramdd(ip, bins=[rbins, vbins, phibins])
...
class SmoothSpine(ExtractProfiles):
    ...
    def FT(self,P, pad_type="odd", pad_len="full", n_interp="mindis"):
        ...
        #deleting the repeated points
        rep=np.where(l==0)[0]
        if len(rep>0):
            print("repetitions seen")
            P=np.delete(P, rep, axis=0)
            l=np.delete(l, rep)
